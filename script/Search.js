...
// .........f?otter............
document.getElementById("Search-Div-box").addEventListener("click",function(){
   
    let inputs=document.getElementById("Search-Div-box")
...
async function mainNews() {
   let Cities= document.getElementById("Search-input").value
   console.log(Cities)
  // console.log("hii");
  try {
    
      let URL = `https://ndtvnews-api.herokuapp.com/cities?city=values(${Cities})&field=values(headline,posted_date,description,url,image_url)`;
      const res = await fetch(URL);
      // console.log(res)
      const data = await res.json()
...
function appendmainNewsData(data){
  ...
  document.getElementById("hidustanNews-mainNews-box").innerHTML=null;
...
function appendnewsadsData(data){
  ...
  data.forEach(el => {
      const box=document.createElement("div")
      const indiaNews=document.createElement("h2")
      indiaNews.innerText="Boxing News"
      indiaNews.style.color="#00b1cd"
